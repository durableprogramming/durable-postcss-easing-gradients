...
let timingFunctions = [...easeShorthands, cubicFunction, stepsFunction, ...saltcatEasings]
...
export async function getTimingFunctions() {
  if (_advancedEasingFunctions === null) {
    try {
      ...
      timingFunctions = [...easeShorthands, cubicFunction, stepsFunction, ..._advancedEasingFunctions]
    } catch (error) {
...
export const isTimingFunctionSync = str => {
  ...
  const allFunctions = [...easeShorthands, cubicFunction, stepsFunction, ...saltcatEasings]
  ...
  if (functionName === cubicFunction) {
    ...
    const params = getParenthesisInsides(str).split(',').map(p => p.trim())
    return params.length === 4 && params.every(p => !isNaN(parseFloat(p)))
  }
  
  if (functionName === stepsFunction) {
    if (!hasParenthesis(str)) return false
    const params = getParenthesisInsides(str).split(',').map(p => p.trim())
    return params.length >= 1 && params.length <= 2 && !isNaN(parseInt(params[0]))
  }
